...
- Trip detail screen with itinerary, map, expenses, packing, and documents tabs (read paths; most non-packing write paths still local-only)
- **Real-time collaborative packing list** — add, toggle, and remove items; updates broadcast to every connected client on the trip via Yjs over WebSocket, with debounced persistence to Postgres
...
- Migrate day / day items / expenses / documents onto the same Yjs doc (packing is the reference panel)
